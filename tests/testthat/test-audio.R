test_that("chunks concatenate to exactly the bytes that were sent", {
  ...
  set.seed(42)   # the payload is arbitrary but must be reproducible
...
test_that("an empty take leaves no file and records no audio", {
  p <- make_project()
  part <- ph_audio_open(p$cfg, "top.jpg", 1L)
...
test_that("an interrupted take leaves a .part and no sidecar audio", {
  ...
  # A zero-byte .part is not an interruption.
